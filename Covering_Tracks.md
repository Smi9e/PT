...
sed -i /192.168.12.1.100/d' /var/log/*tmp 
rm -rf /var/log/* #后果较为严重
```
...
```
chown root:root example.txt #更改文件所有者和组为root和root
```
...
```
~/.mysql_history #mysql历史文件
```

###### 文件覆写

```
shred -f -u -z -v -n 8 1.txt #需要时间较长
